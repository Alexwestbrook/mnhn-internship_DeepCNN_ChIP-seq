...
from deeptools import (
    parserCommon,
    writeBedGraph,  # This should be made directly into a bigWig
)
...
def get_required_args():
    ...
    required.add_argument(
        "--bam", "-b", help="BAM file to process", metavar="BAM file", required=True
    )
...
def get_optional_args():
    ...
    optional.add_argument(
        "--MidPoints",
        help="Determine mid points of paired-end read data. "
        "This is an add-on to default bamCoverage script. "
...
def process_args(args=None):
    ...
    if args.smoothLength and args.smoothLength <= args.binSize:
        print(
            "Warning: the smooth length given ({}) is smaller than the bin "
            "size ({}).\n\n No smoothing will be done".format(
                args.smoothLength, args.binSize
            )
        )
...
def main(args=None):
    ...
    if args.MNase:
        ...
        if frag_len_dict is None:
            sys.exit(
                "*Error*: For the --MidPoints function a paired end library is required. "
            )
...
class MidFragments(writeBedGraph.WriteBedGraph):
    ...
    def get_fragment_from_read(self, read):
        ...
        # Fragments have already been filtered according to length
        if read.is_proper_pair and not read.is_reverse and 1 < abs(read.tlen):
            fragment_start = read.pos + read.tlen // 2
            fragment_end = fragment_start + 1
