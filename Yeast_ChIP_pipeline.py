...
def clean_ratio(ips, ctrls, func='ratio'):
    ...
    if func == 'ratio':
        ...
        def func(x, y): return np.log(1 + x / y)
    elif func == 'KL':
        def func(x, y): return x * np.log(1 + x / y)
    elif func == 'sqrtKL':
        def func(x, y): return np.sqrt(x) * np.log(1 + x / y)
    else:
        assert callable(func)
    return {k: np.where(ctrls[k] == 0, 0, func(ips[k], ctrls[k]))
            for k in ips.keys()}
...
# Merge Control
myco_ctrl_merge = {k: (myco_coh_ctrl[k] + myco_pol_ctrl[k]) / 2
                   for k in myco_coh_ctrl.keys()}
pneu_ctrl_merge = {k: (pneu_coh_ctrl[k] + pneu_pol_ctrl[k]) / 2
                   for k in pneu_coh_ctrl.keys()}

myco_coh = clipnorm(clean_ratio(myco_coh_ip, myco_ctrl_merge, func='sqrtKL'))
myco_pol = clipnorm(clean_ratio(myco_pol_ip, myco_ctrl_merge, func='sqrtKL'))
pneu_coh = clipnorm(clean_ratio(pneu_coh_ip, pneu_ctrl_merge, func='sqrtKL'))
pneu_pol = clipnorm(clean_ratio(pneu_pol_ip, pneu_ctrl_merge, func='sqrtKL'))

np.savez(Path(data_dir, 'labels_myco_coh.npz'), **myco_coh)
np.savez(Path(data_dir, 'labels_myco_pol.npz'), **myco_pol)
np.savez(Path(data_dir, 'labels_pneu_coh.npz'), **pneu_coh)
np.savez(Path(data_dir, 'labels_pneu_pol.npz'), **pneu_pol)

myco_coh = clipnorm(clean_ratio(myco_coh_ip, myco_ctrl_merge, func='KL'))
myco_pol = clipnorm(clean_ratio(myco_pol_ip, myco_ctrl_merge, func='KL'))
pneu_coh = clipnorm(clean_ratio(pneu_coh_ip, pneu_ctrl_merge, func='KL'))
pneu_pol = clipnorm(clean_ratio(pneu_pol_ip, pneu_ctrl_merge, func='KL'))

np.savez(Path(data_dir, 'labels_myco_coh_KL.npz'), **myco_coh)
np.savez(Path(data_dir, 'labels_myco_pol_KL.npz'), **myco_pol)
np.savez(Path(data_dir, 'labels_pneu_coh_KL.npz'), **pneu_coh)
np.savez(Path(data_dir, 'labels_pneu_pol_KL.npz'), **pneu_pol)
