...
def parsing():
    ...
    parser.add_argument(
        "-cobj", "--custom_objects",
        help="custom objects to pass to tf.keras.models.load_model",
        nargs='+',
        type=str,
        default=None)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Value to ivide base learning rate by, default to 10",
        default=10,
        type=float)
    parser.add_argument(
        "-lr", "--learn_rate",
        help="Value for learning rate, overrides learn_rate_decay, default to "
             "None",
        default=None,
        type=float)
    parser.add_argument(
        "-ep", "--epochs",
        help="Maximum number of training loops over the dataset, default to "
             "10",
        default=10,
        type=int)
    parser.add_argument(
    ...
    parser.add_argument(
        "-exdim", "--extradims",
        help="Extra dimensions needed as input for the model, default to None",
        default=None,
        type=int)
    parser.add_argument(
        "-h_int", "--head_interval",
...
if __name__ == "__main__":
    OBJECTS = {"correlate": correlate, "mae_cor": mae_cor}
    ...
    if args.distribute:
        ...
        with strategy.scope():
            model = tf.keras.models.load_model(
                args.model_file,
                custom_objects={k: OBJECTS[k] for k in args.custom_objects})
    else:
        model = tf.keras.models.load_model(
            args.model_file,
            custom_objects={k: OBJECTS[k] for k in args.custom_objects})
    ...
    if args.learn_rate is None:
        model.optimizer.lr = model.optimizer.lr / args.learn_rate_decay
    else:
        model.optimizer.lr = args.learn_rate
    # Load the data
