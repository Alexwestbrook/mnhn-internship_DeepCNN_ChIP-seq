...
def get_profile(seqs, model, winsize, head_interval, reverse=False,
    ...
    if stride != 1:
        ...
        # Determine number of slides for reshaping, and allow higher batch_size
...
def select(energies, weights, temperature, step=None):
    ...
    if step is not None:
        np.save(Path(args.output_dir, f'prob_step{step}.npy'), prob)
...
def main(args):
    ...
    for step in range(args.steps):
        ...
        np.save(Path(args.output_dir, f"mut_seqs_step{step}.npy"), seqs)
        with open(Path(args.output_dir, 'energy.txt'), 'a') as f:
            f.write(f'{sel_energies}\n')
        fig, axes = plt.subplots(nrow, ncol, figsize=(2+3*nrow, 1+2*ncol),
        ...
        if args.n_seqs == 1:
            ...
            for ax, pfor, prev in zip(axes.flatten(),
                ...
                ax.plot(np.arange(offset, len(prev)+offset),
                        prev, label='reverse', alpha=0.8)
                ax.legend()
        fig.savefig(Path(args.output_dir, f"mut_preds_step{step}.png"),
                    bbox_inches='tight')
        plt.close()
        print(time.time() - t0)
